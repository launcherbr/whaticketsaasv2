...
const VersionLog = () => {
    ...
    useEffect(() => {
        const fetchReadme = async () => {
            try {
                const response = await axios.get("https://github.com/packtypebot/logs.git");
    ...
    const formatMarkdown = (text) => {
        ...
        let formatted = text.replace(
            /\[([^\]]+)\]\(([^)]+)\)/g,
            '<a href="$2" target="_blank" rel="noopener noreferrer" style="color: #3498db; text-decoration: none;">$1</a>'
        );
        ...
        formatted = formatted.replace(
            /`([^`]+)`/g,
            '<code style="background: #f5f5f5; padding: 0.2em 0.4em; border-radius: 3px; font-family: monospace;">$1</code>'
        );
                ...
                <VersionCard key={version}>
                    ...
                    <ChangeList>
                        {changes.map((change, index) => (
                            <ChangeItem key={index} dangerouslySetInnerHTML={{ __html: change }} />
                        ))}
                    </ChangeList>
                </VersionCard>
            ))}
